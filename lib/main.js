...
const parseVideoInfo = (item) => {
    ...
    const uploaded_at_text = isLiveStream ? null : chrono.parseDate(getTextFromObject(item['publishedTimeText']));
    ...
    return {
        ...
        title: title,
        description: !item['descriptionSnippet'] ? "" : getTextFromObject(item['descriptionSnippet']),
...
const parseItems = (response) => {
    ...
    let itemSectionRenderers = response['response']['contents']['twoColumnBrowseResultsRenderer']
        ['tabs'][0]['tabRenderer']['content']['sectionListRenderer']['contents'];
    itemSectionRenderers.forEach((itemSectionRenderer) => {
        let renderers = itemSectionRenderer['itemSectionRenderer']['contents'][0]
        let expandedShelfContentsRenderer = Object.keys(renderers).includes('shelfRenderer') ? renderers['shelfRenderer']['content']['expandedShelfContentsRenderer'] : null
